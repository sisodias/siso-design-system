...
const importsOut = path.join(ROOT, 'viewer', 'lib', 'preview-imports.generated.ts')
const renderableComponents = manifest.components.filter(c => c.preview?.renderable !== false && c.source === '21st-dev')
const importLines = [
  '// AUTO-GENERATED by scripts/build-manifest.mjs. DO NOT EDIT.',
  '// Regenerate via `node scripts/build-manifest.mjs` (runs on prebuild and predev).',
  "import type { ComponentType } from 'react'",
  '',
  'export type PreviewLoader = () => Promise<{ default: ComponentType<Record<string, unknown>> } | Record<string, unknown>>',
  '',
  'export const previewLoaders: Record<string, PreviewLoader> = {',
]
for (const c of renderableComponents) {
  const key = `${c.source}/${c.name}`
  importLines.push(`  ${JSON.stringify(key)}: () => import(${JSON.stringify(`@lib/${c.source}/${c.name}/demo`)}),`)
}
importLines.push('}')
importLines.push('')
writeFileSync(importsOut, importLines.join('\n') + '\n')
console.log(`  Preview loader map: ${renderableComponents.length} entries → ${path.relative(ROOT, importsOut)}`)
console.log(
