...
interface MultiCategoryDataPoint {
  key: string; // e.g., 'Q1', 'Q2'
  data: Array<{
    key: string; // Series name, e.g., 'XML', 'JSON'
    data: number | null;
  }>;
}

const multiCategorySmallBlockRaw: MultiCategoryDataPoint[] = [
  {
    key: 'Jan',
    data: [
      { key: 'XML', data: 50 },
...
const validatedMultiCategorySmallBlock = multiCategorySmallBlockRaw.map(category => ({
  ...category,
  data: category.data.map(seriesItem => ({
    ...seriesItem,
    data: (typeof seriesItem.data === 'number' && !isNaN(seriesItem.data)) ? seriesItem.data : 0,
  })),
}));
...
interface GroupedIncidentReportCardProps {
  // Props can be added here for customization
}

function GroupedIncidentReportCard({}: GroupedIncidentReportCardProps): JSX.Element {
  return (
      ...
      <div className="flex justify-between w-full px-8 mb-4">
        {legendItems.map(item => (
          <div key={item.name} className="flex gap-2 items-center">
            <div className="w-3 h-3 sm:w-4 sm:h-4" style={{ backgroundColor: item.color }} />
            <span className="text-neutral-600 dark:text-[#9A9AAF] text-xs">{item.name}</span>
          </div>
        ))}
      </div>
      <div className="flex-grow px-4 h-[200px]"> {/* Explicit height for chart container */}
        <BarChart
          id="grouped-incident-report-chart"
          height={200} // Explicit height for the chart itself
          data={validatedMultiCategorySmallBlock}
          yAxis={
            <LinearYAxis
              axisLine={null}
          ...
          series={
            <BarSeries
              ...
              layout="vertical"
              type="grouped"
              groupPadding={15} // Adjusted groupPadding for better fit
            />
          }
          gridlines={
            <GridlineSeries
...
export default GroupedIncidentReportCard;