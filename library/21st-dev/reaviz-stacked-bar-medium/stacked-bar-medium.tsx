...
import {
  ...
  GridlineSeries,
  Gridline,
  ChartDataTypes, // For stacked bar chart data
} from 'reaviz';

// Type definitions
interface StackedBarChartDataPoint {
  key: string; // Category name (e.g., 'Mon', 'Tue')
...
// Data and Constants
// Sample data for a stacked bar chart.
// Each object in the array represents a category on the X-axis.
// The 'data' array within each object represents the different series stacked for that category.
const initialMultiCategoryData: StackedBarChartDataPoint[] = [
  {
    key: 'Mon',
    data: [
      { key: 'Series A', data: 30 }, { key: 'Series B', data: 20 }, { key: 'Series C', data: 15 }, { key: 'Series D', data: 10 }
    ],
  },
  {
    key: 'Tue',
    data: [
      { key: 'Series A', data: 40 }, { key: 'Series B', data: 25 }, { key: 'Series C', data: 10 }, { key: 'Series D', data: 5 }
    ],
  },
  {
    key: 'Wed',
    data: [
      { key: 'Series A', data: 25 }, { key: 'Series B', data: 30 }, { key: 'Series C', data: 20 }, { key: 'Series D', data: 12 }
    ],
  },
  {
    key: 'Thu',
    data: [
      { key: 'Series A', data: 35 }, { key: 'Series B', data: 15 }, { key: 'Series C', data: 25 }, { key: 'Series D', data: 8 }
    ],
  },
  {
    key: 'Fri',
    data: [
      { key: 'Series A', data: 45 }, { key: 'Series B', data: 22 }, { key: 'Series C', data: 18 }, { key: 'Series D', data: 15 }
    ],
  },
];

// Data validation utility for StackedBarChartData
const validateStackedBarChartData = (data: StackedBarChartDataPoint[]): ChartDataTypes[] => {
...
const validatedChartData = validateStackedBarChartData(initialMultiCategoryData);

const BAR_CHART_COLOR_SCHEME = ['#4C86FF', '#40E5D1', '#40D3F4', '#9152EE'];
...
const DetailedStackedBarReport: React.FC = () => {
  return (
    <>
      <style jsx global>{`
        :root {
      ...
      <div className="flex flex-col pt-4 pb-4 bg-white dark:bg-black rounded-3xl shadow-[11px_21px_3px_rgba(0,0,0,0.06),14px_27px_7px_rgba(0,0,0,0.10),19px_38px_14px_rgba(0,0,0,0.13),27px_54px_27px_rgba(0,0,0,0.16),39px_78px_50px_rgba(0,0,0,0.20),55px_110px_86px_rgba(0,0,0,0.26)] w-full max-w-md min-h-[580px] overflow-hidden transition-colors duration-300">
        ...
        <div className={'flex-grow px-4 reaviz-chart-container'}>
          <StackedBarChart
            height={200} // Adjusted height
            id="detailed-stacked-bar"
...
export default DetailedStackedBarReport;