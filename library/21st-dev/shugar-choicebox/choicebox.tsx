...
ChoiceboxGroup.Item = ({
  ...
  return (
    <div
      className={`border ${isSelected ? "border-blue-600" : "border-gray-400"} w-full rounded-md duration-150 ${disabled ? "cursor-not-allowed" : "cursor-pointer"} ${isSelected ? "bg-blue-100" : "bg-transparent"}`}
      onClick={onClick}
    >
      <div className="flex items-center gap-4 p-3">
        <div className="flex flex-col gap-1 font-sans text-sm">
          <span
            className={`font-medium ${disabled ? "text-gray-500" : isSelected ? "text-blue-900" : "text-gray-1000"}`}>{title}</span>
          <span
            className={disabled ? " text-gray-500" : isSelected ? "text-blue-900" : "text-gray-900"}>{description}</span>
        </div>
