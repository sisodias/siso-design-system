...
const baseSimpleFunnelDataRaw: FunnelDataPoint[] = [
...
const validatedBaseSimpleFunnelData = baseSimpleFunnelDataRaw.map(item => ({
  ...item,
  data: (typeof item.data === 'number' && !isNaN(item.data)) ? item.data : 0,
}));

// Color scheme
const funnelChartColor = '#40D3F4'; // Single color for this interpolated funnel
    ...
    trendIconSvg: (
      <div className="flex bg-[rgb(232,64,69)]/40 p-1 pl-2 pr-2 items-center rounded-full text-[#F08083]">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21" fill="none">
          <path d="M5.50134 9.11119L10.0013 4.66675M10.0013 4.66675L14.5013 9.11119M10.0013 4.66675L10.0013 16.3334" stroke="#F08083" strokeWidth="2" strokeLinecap="square" />
        </svg>
        12%
      </div>
    ),
  ...
  },
  {
    ...
    trendIconSvg: (
      <div className="flex bg-[rgb(64,229,209)]/40 p-1 pl-2 pr-2 items-center rounded-full text-[#40E5D1]">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21" fill="none">
          <path d="M14.4987 11.8888L9.99866 16.3333M9.99866 16.3333L5.49866 11.8888M9.99866 16.3333V4.66658" stroke="#40E5D1" strokeWidth="2" strokeLinecap="square" />
        </svg>
         4%
      </div>
    ),
...
interface InterpolatedFunnelReportCardProps {
  title?: string;
}

function InterpolatedFunnelReportCard({ title = "Incident Report" }: InterpolatedFunnelReportCardProps): JSX.Element {
  ...
  const chartData = useMemo(() => {
    // Simulate data change based on time range for demonstration
    let dataSet = validatedBaseSimpleFunnelData;
    if (timeRange === 'last-7-days') {
      dataSet = validatedBaseSimpleFunnelData.map(item => ({
        ...item,
        data: Math.max(0, (item.data || 0) - Math.floor(Math.random() * 50 + 50)) // Reduce values for "Last 7 days"
      })).filter(item => (item.data || 0) > 0); // Filter out zero/negative data points for funnel
    } else if (timeRange === 'last-30-days') {
       dataSet = validatedBaseSimpleFunnelData.map(item => ({
        ...item,
        data: (item.data || 0) + Math.floor(Math.random() * 20) // Slightly adjust for "Last 30 days"
      }));
    }
    // For 'last-90-days', use the full dataset or fetch/generate more
    return dataSet.length > 1 ? dataSet : validatedBaseSimpleFunnelData; // Ensure at least 2 points for funnel
  }, [timeRange]);
  ...
  return (
    <div className="flex flex-col justify-between pt-4 pb-4 bg-white dark:bg-black rounded-3xl shadow-[11px_21px_3px_rgba(0,0,0,0.06),14px_27px_7px_rgba(0,0,0,0.10),19px_38px_14px_rgba(0,0,0,0.13),27px_54px_27px_rgba(0,0,0,0.16),39px_78px_50px_rgba(0,0,0,0.20),55px_110px_86px_rgba(0,0,0,0.26)] w-[600px] h-[714px] overflow-hidden transition-colors duration-300">
      <div className="flex justify-between items-center p-7 pt-6 pb-8">
        <h3 className="text-3xl text-left font-bold text-neutral-800 dark:text-white">
          {title}
        </h3>
        <select
          value={timeRange}
          ...
          className="bg-neutral-200 dark:bg-[#262631] text-neutral-800 dark:text-white p-3 pt-2 pb-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
      ...
      <div className="flex-grow px-4 h-[250px]"> {/* Explicit height for chart container */}
        <FunnelChart
          id="interpolated-funnel-chart"
          height={250} // Explicit height for the chart itself
          data={chartData}
          series={
            <FunnelSeries
              arc={
                <FunnelArc
                  colorScheme={funnelChartColor} // Single color for interpolated
                  interpolation="step"
                  gradient={null}
                  tooltip={<TooltipArea />}
                  glow={{
                    blur: 20, // Adjusted blur
                    color: `${funnelChartColor}99`, // Glow color with opacity
                  }}
                />
              }
              axis={
                <FunnelAxis
                  label={
                    <FunnelAxisLabel 
                      ...
                      className="font-semibold text-neutral-600 dark:text-neutral-400" // Themed text color
                    />
                  }
                  line={
                    <FunnelAxisLine 
                      strokeColor="currentColor" // Use currentColor for theme compatibility
                      className="text-neutral-400 dark:text-neutral-600" // Themed line color
                    />
                  }
                />
              }
      ...
      <div className="flex w-full pl-8 pr-8 justify-between pb-2 pt-3">
        <div className="flex flex-col gap-2 w-1/2">
          <span className="text-xl text-neutral-700 dark:text-neutral-300">Critical Incidents</span>
          <div className="flex items-center gap-2">
            <Count
              className="font-mono text-4xl font-semibold text-neutral-800 dark:text-white"
              from={0}
              to={criticalIncidentsData.count}
            />
            <div className="flex bg-[rgb(232,64,69)]/40 p-1 pl-2 pr-2 items-center rounded-full text-[#F08083]">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21" fill="none">
                <path d="M5.50134 9.11119L10.0013 4.66675M10.0013 4.66675L14.5013 9.11119M10.0013 4.66675L10.0013 16.3334" stroke="#F08083" strokeWidth="2" strokeLinecap="square" />
          ...
          <span className="text-neutral-500 dark:text-[#9A9AAF] text-sm">
            {criticalIncidentsData.comparisonText}
          </span>
        </div>
        <div className="flex flex-col gap-2 w-1/2">
          <span className="text-xl text-neutral-700 dark:text-neutral-300">Total Incidents</span>
          <div className="flex items-center gap-2">
            <Count
              className="font-mono text-4xl font-semibold text-neutral-800 dark:text-white"
              from={0}
              to={totalIncidentsData.count}
            />
            <div className="flex bg-[rgb(64,229,209)]/40 p-1 pl-2 pr-2 items-center rounded-full text-[#40E5D1]">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21" fill="none">
                <path d="M14.4987 11.8888L9.99866 16.3333M9.99866 16.3333L5.49866 11.8888M9.99866 16.3333V4.66658" stroke="#40E5D1" strokeWidth="2" strokeLinecap="square" />
      ...
      <div className="flex flex-col pl-8 pr-8 font-mono divide-y divide-neutral-200 dark:divide-[#262631]">
        {metrics.map(metric => (
          <motion.div
            key={metric.id}
