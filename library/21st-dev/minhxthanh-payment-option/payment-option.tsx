...
const RadioOption: React.FC<RadioOptionProps> = ({ option, name, isChecked, onChange }) => {
  return (
    <label
      className="group relative flex w-full cursor-pointer items-center justify-between overflow-hidden rounded-lg border border-transparent p-3 transition-all duration-500 hover:bg-slate-100 dark:hover:bg-slate-800 has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50 dark:has-[:checked]:bg-indigo-950 has-[:checked]:font-semibold has-[:checked]:text-indigo-900 dark:has-[:checked]:text-indigo-100"
    ...
    >
      ...
      <input
        ...
        className="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-0"
...
const PaymentRadio: React.FC<PaymentRadioProps> = ({
  ...
  return (
    <div className={`w-full max-w-sm space-y-1 dark:bg-neutral-900 p-4 shadow-xl border-0 rounded-3xl px-8 py-8 bg-neutral-100 ${className}`}>
      <div className="mb-4">
        <h3 className="font-semibold text-gray-900 dark:text-gray-100">Payment Method</h3>
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Choose your preferred payment option</p>
      </div>
