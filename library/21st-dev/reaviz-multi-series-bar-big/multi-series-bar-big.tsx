...
import {
  ...
  GridlineSeries,
  Gridline,
  ChartDataTypes, // For grouped bar chart data
} from 'reaviz';
...
const LEGEND_ITEMS: LegendItem[] = [
  { name: 'XML', color: '#DAC5F9' },
  { name: 'JSON', color: '#40E5D1' },
  { name: 'HTTPS', color: '#9152EE' },
  { name: 'SSH', color: '#5B14C5' },
];

const BAR_CHART_COLOR_SCHEME = ['#DAC5F9', '#40E5D1', '#9152EE', '#5B14C5'];
...
const initialMultiCategorySmallBlock: GroupedBarChartDataPoint[] = [
  {
    key: 'Firewall',
    data: [
      { key: 'XML', data: 20 }, { key: 'JSON', data: 30 }, { key: 'HTTPS', data: 45 }, { key: 'SSH', data: 15 },
    ],
  },
  {
    key: 'Endpoint',
    data: [
      { key: 'XML', data: 25 }, { key: 'JSON', data: 35 }, { key: 'HTTPS', data: 50 }, { key: 'SSH', data: 10 },
    ],
  },
  {
    key: 'Network',
    data: [
      { key: 'XML', data: 15 }, { key: 'JSON', data: 25 }, { key: 'HTTPS', data: 40 }, { key: 'SSH', data: 20 },
    ],
  },
  {
    key: 'Cloud',
    data: [
      { key: 'XML', data: 30 }, { key: 'JSON', data: 40 }, { key: 'HTTPS', data: 55 }, { key: 'SSH', data: 25 },
    ],
  },
];
...
const validatedChartData = validateGroupedBarChartData(initialMultiCategorySmallBlock);
...
const DetailedGroupedBarReport: React.FC = () => {
  ...
  return (
    <>
      ...
      <div className="flex flex-col justify-between pt-4 pb-4 bg-white dark:bg-black rounded-3xl shadow-[11px_21px_3px_rgba(0,0,0,0.06),14px_27px_7px_rgba(0,0,0,0.10),19px_38px_14px_rgba(0,0,0,0.13),27px_54px_27px_rgba(0,0,0,0.16),39px_78px_50px_rgba(0,0,0,0.20),55px_110px_86px_rgba(0,0,0,0.26)] w-full max-w-2xl min-h-[714px] overflow-hidden transition-colors duration-300">
        ...
        {/* Legend */}
        <div className="flex flex-wrap gap-x-8 gap-y-2 w-full pl-8 pr-8 mb-4">
        ...
        {/* Bar Chart */}
        <div className="reaviz-chart-container h-[280px] px-4 mb-4">
          <BarChart
            height={280}
            id="detailed-grouped-bar"
                  ...
                  <LinearXAxisTickSeries
                    label={
                      <LinearXAxisTickLabel
                        padding={10}
                        rotation={-45}
                        format={text => typeof text === 'string' ? `${text.slice(0, 8)}...` : ''}
            ...
            series={
              <BarSeries
                type="grouped"
                layout="vertical"
                bar={<Bar width={8} glow={{ blur: 20, opacity: 0.7 }} gradient={null} />}
                colorScheme={BAR_CHART_COLOR_SCHEME}
                groupPadding={50} // As per original `groupPadding={50}`
              />
            }
...
export default DetailedGroupedBarReport;