  ...
  const diff = useMemo(
    () =>
      [
        ...
        { kind: "context", old: 12, new: 12, content: "export function getUserName(id: string) {" },
        { kind: "del", old: 13, new: null, content: "  const name = cache.get(id)" },
        { kind: "add", old: null, new: 13, content: "  const name = cache.get(id) ?? '(unknown)'" },
        { kind: "context", old: 14, new: 14, content: "  return name?.trim()" },
        { kind: "context", old: 15, new: 15, content: "}" },
        { kind: "hunk", content: "@@ -32,4 +34,6 @@" },
        { kind: "context", old: 32, new: 34, content: "const PORT = process.env.PORT || 3000" },
        { kind: "del", old: 33, new: null, content: "server.listen(PORT)" },
        { kind: "add", old: null, new: 35, content: "server.listen(PORT, () => {" },
        { kind: "add", old: null, new: 36, content: "  console.log('Server running on', PORT)" },
  ...
  return (
    <main className="mx-auto max-w-4xl p-4">
      <header className="mb-3 flex items-start justify-between gap-3">
        <div>
          <h1 className="text-pretty text-lg font-semibold">Inline Diff Comments (GitHub-style)</h1>
          <p className="mt-0.5 text-xs text-muted-foreground">
            Hover a line to reveal “Add comment”. Click to open a compact inline thread.
          </p>
        </div>
      </header>
...
function DiffList({
  ...
  return (
    <TooltipProvider delayDuration={150}>
      <div role="table" aria-label={`Diff of ${fileName}`} className="rounded-md border bg-card dark:border-white/10">
          ...
          {rows.map((line, idx) => {
            ...
            return (
              ...
              >
                ...
                >
                  <span className="px-2 py-1 tabular-nums text-right">
                ...
                <div role="cell" className="flex-1">
                  <pre
                    className={cn("whitespace-pre-wrap px-2 py-1 font-mono text-[12px] leading-5", isChange && "pl-5")}
    ...
    <div className="rounded-md border bg-card dark:border-white/10">
      ...
      <div className="flex items-center justify-between gap-2 px-2 py-1">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Button variant="ghost" size="icon" aria-label="Close thread" onClick={onClose} className="h-7 w-7">
            <X className="h-4 w-4" />
          </Button>
        </div>
      </div>
          ...
          <li key={c.id} className="flex items-start gap-2">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="ml-2 shrink-0 text-[10px] text-muted-foreground">{c.createdAt}</span>
              </div>
      ...
      <div className="flex flex-col gap-2 px-2 py-1.5">
        ...
        <div className="flex items-center justify-end gap-1.5">
          ...
          <Button size="sm" onClick={addComment} disabled={!draft.trim()} className="h-7 px-2 text-[12px]">
            Comment
          </Button>
        </div>
      </div>
