...
interface IncidentReportCardSmallProps {
  isDarkMode?: boolean;
}

export const Component = ({ isDarkMode = false }: IncidentReportCardSmallProps) => {
  const areaSingleSeriesSimpleData = useMemo(() => generateChartData(7), []);
  ...
  const metricIconFill = isDarkMode ? "#E84045" : "#EF4444"; // Red-500 for light

  const getBadgeColors = (badgeType: "critical" | "improvement") => {
    if (badgeType === "critical") {
      return {
        rectFill: isDarkMode ? "rgb(232 64 69)" : "rgb(254 226 226)", // red-100 for light
        rectOpacity: isDarkMode ? 0.4 : 1,
        arrowStroke: isDarkMode ? "#F08083" : "rgb(220 38 38)", // red-600 for light
        arrowPath: "M9.50134 12.6111L14.0013 8.16663M14.0013 8.16663L18.5013 12.6111M14.0013 8.16663L14.0013 19.8333",
      };
    } else { // improvement
      return {
        rectFill: isDarkMode ? "rgb(64 229 209)" : "rgb(204 251 241)", // teal-100 for light
        rectOpacity: isDarkMode ? 0.4 : 1,
        arrowStroke: isDarkMode ? "#40E5D1" : "rgb(13 148 136)", // teal-600 for light
  ...
  return (
    <div
      className={cn(
        "flex flex-col pt-4 pb-4 rounded-3xl overflow-hidden w-[375px] h-[560px]",
    ...
    >
      <h3 className="text-3xl text-left p-7 pt-6 pb-8 font-bold">
        Incident Report
      </h3>
              ...
              area={
                <Area
                  gradient={
                    <Gradient
                      stops={[
                        <GradientStop key="g_stop_0" stopOpacity={0} />,
                        <GradientStop key="g_stop_1" offset="100%" stopOpacity={gradientStopOpacity} />,
                      ]}
                    />
                  }
                />
      ...
      <div className="flex flex-col pl-8 pr-8 pt-8 font-mono divide-y divide-gray-200 dark:divide-[#262631]">
