...
const baseMultiCategoryRaw: ChartCategoryData[] = [
  {
    key: 'Phishing',
    data: [
      { key: 'Low', data: 20 },
...
const validatedBaseMultiCategoryData = baseMultiCategoryRaw.map(category => ({
  ...category,
...
const chartColors = ['#4C86FF', '#40E5D1', '#40D3F4', '#9152EE']; // Low, Medium, High, Critical
...
interface ComprehensiveIncidentReportCardProps {
  title?: string;
}

function ComprehensiveIncidentReportCard({ title = "Incident Report" }: ComprehensiveIncidentReportCardProps): JSX.Element {
  ...
  const chartData = useMemo(() => {
    ...
    let dataSet = validatedBaseMultiCategoryData;
    if (timeRange === 'last-7-days') {
      dataSet = validatedBaseMultiCategoryData.slice(0, 3).map(cat => ({
        ...cat,
        data: cat.data.map(seg => ({ ...seg, data: Math.max(0, (seg.data || 0) - Math.floor(Math.random() * 10)) }))
      }));
    } else if (timeRange === 'last-30-days') {
       dataSet = validatedBaseMultiCategoryData.slice(0, 4).map(cat => ({
        ...cat,
        data: cat.data.map(seg => ({ ...seg, data: (seg.data || 0) + Math.floor(Math.random() * 5) }))
      }));
    }
  ...
  return (
    <div className="flex flex-col justify-between pt-4 pb-4 bg-white dark:bg-black rounded-3xl shadow-[11px_21px_3px_rgba(0,0,0,0.06),14px_27px_7px_rgba(0,0,0,0.10),19px_38px_14px_rgba(0,0,0,0.13),27px_54px_27px_rgba(0,0,0,0.16),39px_78px_50px_rgba(0,0,0,0.20),55px_110px_86px_rgba(0,0,0,0.26)] w-[600px] h-[714px] overflow-hidden transition-colors duration-300">
      ...
      <div className="flex-grow px-4 h-[250px]"> {/* Explicit height for chart container */}
        <StackedBarChart
          id="comprehensive-stacked-incident-chart"
...
export default ComprehensiveIncidentReportCard;