...
function DetailedIncidentReportCard({}: DetailedIncidentReportCardProps): JSX.Element {
  ...
  // Example: Filter or modify chart data based on timeRange if needed
  // For this example, we'll use the static validatedBaseCategoryData
  const chartData = useMemo(() => {
    // Here you could, for example, slice the data or fetch new data
    // based on `timeRange`. For simplicity, we'll return the full set.
    if (timeRange === 'last-7-days') {
      return validatedBaseCategoryData.slice(0, 4); // Show fewer categories for demo
    }
        ...
        <BarChart
            ...
            <LinearYAxis
              ...
              tickSeries={
                <LinearYAxisTickSeries
                  label={
                    <LinearYAxisTickLabel
                      format={(text: string) => (text.length > 10 ? `${text.slice(0,10)}...` : text)} // Adjusted slice length
          ...
          series={
            <BarSeries
              ...
              padding={0.1} // Kept original padding
            />
          }
          gridlines={
            <GridlineSeries
