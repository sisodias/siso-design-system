...
import {
  ...
  GridlineSeries,
  Gridline,
  ChartData, // Reaviz type for simple chart data
} from 'reaviz';
...
// Sample data for the BarChart - assumed to be an array of ChartData objects
const initialBarChartData: ChartData[] = [
  { key: 'Firewall', data: 65 },
  { key: 'IDS/IPS', data: 50 },
  { key: 'Endpoint', data: 75 },
  { key: 'SIEM', data: 40 },
  { key: 'Email Sec', data: 80 },
  { key: 'Web Sec', data: 55 },
  { key: 'Cloud Sec', data: 70 },
];
...
const validatedBarChartData = validateBarChartData(initialBarChartData);
...
const DETAILED_METRICS_DATA: DetailedMetric[] = [
  ...
  },
  {
    ...
    value: '10%',
    TrendIcon: DetailedTrendDownIcon, // Assuming lower is better for escalation rate
...
const AdvancedIncidentReportBarChart: React.FC = () => {
  ...
  // For this example, it only updates the select's value and chart data is static
  ...
  return (
    <>
      ...
      <div className="flex flex-col justify-between pt-4 pb-4 bg-white dark:bg-black rounded-3xl shadow-[11px_21px_3px_rgba(0,0,0,0.06),14px_27px_7px_rgba(0,0,0,0.10),19px_38px_14px_rgba(0,0,0,0.13),27px_54px_27px_rgba(0,0,0,0.16),39px_78px_50px_rgba(0,0,0,0.20),55px_110px_86px_rgba(0,0,0,0.26)] w-full max-w-2xl min-h-[714px] overflow-hidden transition-colors duration-300">
        ...
        {/* Bar Chart */}
        <div className="reaviz-chart-container h-[280px] px-4 mb-4"> {/* Added padding for labels */}
          <BarChart
            height={280}
            id="advanced-bar-chart"
            data={validatedBarChartData}
            yAxis={
              <LinearYAxis
                axisLine={null}
              ...
              <BarSeries
                bar={
                  <Bar
                    ...
                    gradient={null} // As per original source, no gradient on bars
                  />
                }
...
export default AdvancedIncidentReportBarChart;