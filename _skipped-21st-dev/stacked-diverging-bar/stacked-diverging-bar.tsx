...
interface DivergingSegment {
  key: string; // Series name, e.g., 'Resolved', 'Outstanding'
  data: number | null;
}

interface ChartCategoryData {
  key: string; // Category on X-axis, e.g., 'Q1', 'Q2'
  data: DivergingSegment[];
}

const binnedDateDataRaw: ChartCategoryData[] = [
  {
    key: 'Jan',
    data: [
      { key: 'Resolved', data: 50 },
      { key: 'Outstanding', data: -20 },
    ],
  },
  {
    key: 'Feb',
    data: [
      { key: 'Resolved', data: 75 },
      { key: 'Outstanding', data: -30 },
    ],
  },
  {
    key: 'Mar',
    data: [
      { key: 'Resolved', data: 60 },
      { key: 'Outstanding', data: -15 },
    ],
  },
  {
    key: 'Apr',
    data: [
      { key: 'Resolved', data: 80 },
      { key: 'Outstanding', data: -25 },
    ],
  },
  {
    key: 'May',
    data: [
      { key: 'Resolved', data: 40 },
      { key: 'Outstanding', data: -10 },
    ],
  },
];
...
const chartColors = ['#F7BFC1', '#E84045']; // Typically light for positive, dark for negative, or vice-versa. Given colors suggest this.
...
interface DivergingIncidentReportCardProps {
  title?: string;
}

function DivergingIncidentReportCard({ title = "Incident Report" }: DivergingIncidentReportCardProps): JSX.Element {
  return (
...
export default DivergingIncidentReportCard;
