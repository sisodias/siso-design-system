...
import { addTag, getTagsForComponent } from '@/lib/curation'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const error = status === 503 ? 'Tag system unavailable in cloud deployment (local-only feature)' : 'Failed to add tag'
    return NextResponse.json(
      { error },
